...
if npx postcss css/**/*.css --dir css/autoprefixed --map; then
...
if npx pa11y-ci --config pa11y.config.js; then
...
if npx lighthouse http://localhost:4000 --output=json --output-path=./lighthouse-compatibility.json --chrome-flags="--headless" --quiet; then
...
BEM_ISSUES=$(grep -r "class.*btn" . --include="*.html" | grep -v "btn--" | grep -v ".min." | awk 'END{print NR}')
BEM_ISSUES=${BEM_ISSUES:-0}
...
if npx stylelint "css/**/*.css" --config .stylelintrc.json --formatter verbose; then
...
if npx pa11y http://localhost:4000 --standard WCAG2AA --reporter json >html-validation.json; then
